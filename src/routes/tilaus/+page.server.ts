...
import { type PaytrailHeaders, calculateHmac, hmacCheck } from '$lib/server/hmac';
...
export const actions: Actions = {
  default: async ({ url, request }) => {
    ...
    // const palvelin = 'https://puoti-production.up.railway.app'
    const body = {
      stamp: tilaus_db.id,
      reference: 'helmi',
      amount,
    ...
    if (!hmacCheck(p_env.PAYTRAIL_SECRET, response.headers, r_body)) {
      errors.push('Maksupalvelun antama vastaus ei ole oikein allekirjoitettu')
    }
    if (!response.ok) {
      errors.push('Virhe maksun lähetyksessä: ' + response.statusText)
    }
    return {
      email, amount, errors,
      providers: r_body.providers
    }
  }
}